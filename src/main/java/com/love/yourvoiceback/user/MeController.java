...
public class MeController {
    ...
    public ResponseEntity<List<SocialAccountResponse>> socialAccounts(
            @CurrentUser User user
    ) {
        return ResponseEntity.ok(userService.getSocialAccounts(user.getId()));
    }

    @DeleteMapping
    @Operation(summary = "현재 로그인한 사용자 계정을 삭제합니다.")
    public ResponseEntity<Void> deleteMe(@CurrentUser User user) {
